# DashCam Router Movie RO Download (Old Draft) - DashCam-Router-Movie-RO_Old_Old.sh

**Deprecated** — an earlier draft of the RO-video downloader, superseded by [../DashCam-Router-Movie-RO.sh](../DashCam-Router-Movie-RO.md) (which uses a PID-file lock instead of the `ps`-grep check below). Kept here for reference.

The file previously had a stray chat prompt pasted at the top and a malformed shebang (`!/bin/bash` missing its `#`), which meant it couldn't run; both have been cleaned up so the script now executes correctly.

Downloads new "RO" DashCam video files from the router's web-exposed `/DCIM/Movie/RO` folder, guarding against overlapping runs with a `ps`-grep check, gated on an `nmap` port-21 check.

## How it works

1. Sets configuration: target device IP, remote video path, local download directory, and a `ps`-grep pattern (`[D]CIM/Movie/RO`, bracket trick to avoid the grep matching itself) used to detect other running instances.
2. Counts running processes matching that pattern. If none found, continues; otherwise assumes another instance is running and skips.
3. Ensures the download directory exists.
4. Checks if port 21 (FTP) is open via `nmap`; if so, fetches the HTML directory listing, extracts `.MP4` filenames matching the DashCam naming pattern, and downloads any not already present locally.
5. Sets a `SIGTERM` trap to hard-kill the process.
