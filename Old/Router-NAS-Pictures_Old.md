# Router to NAS Pictures Sync (Old) - Router-NAS-Pictures_Old.sh

**Deprecated** — superseded by [../Router-NAS-Pictures.sh](../Router-NAS-Pictures.md), which switched from plaintext `sshpass -f /root/Adm/pw.txt` FTP-style rsync to the named-pipe password approach over SSH. Kept here for reference.

Syncs locally stored DashCam photos to the NAS over SFTP, gated on an `nmap` port-22 check.

## How it works

1. Backgrounds a no-op `nohup ls &` and defines the NAS target host.
2. Checks if port 22 (SSH) is open on the NAS via `nmap`.
3. If open, runs `sshpass -f /root/Adm/pw.txt rsync -avz --ignore-existing` to copy `/mnt/sda1/DCIM/Photo` to the NAS's `Ftp/DashCam` share over SFTP.
