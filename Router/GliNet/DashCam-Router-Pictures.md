# DashCam Router Pictures Download - DashCam-Router-Pictures.sh

This script downloads new photo files from the router's web-exposed `/DCIM/Photo` folder to local storage, using a PID file to prevent overlapping runs.
...
5. Fetches the HTML directory listing via `wget`, extracts `.JPG` filenames matching the DashCam naming pattern with `grep`/`sed`, and downloads any file not already present locally.
6. Exits 0 on completion; the `EXIT` trap logs the final status and removes the PID file.

## Usage

Intended to run periodically (e.g. via cron) on the GL.iNet router to pull new DashCam photos before they're overwritten on the device.
