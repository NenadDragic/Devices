# DashCam Router Pictures Download (Old) - DashCam-Router-Pictures_Old.sh

**Deprecated** — superseded by [../DashCam-Router-Pictures.sh](../DashCam-Router-Pictures.md), which replaced the `ps`-based duplicate check with a proper PID-file lock and added trap-based cleanup. Kept here for reference.

Earlier version that downloads new photo files from the router's web-exposed `/DCIM/Photo` folder, guarding against overlapping runs by grepping the process list instead of using a PID file.

## How it works

1. Sets configuration: target device IP, remote photo path, local download directory, and a `ps`-grep pattern used to detect other running instances.
2. Counts running processes matching `[D]CIM/Photo` (the bracket trick avoids the grep matching itself). If none found, continues; otherwise assumes another instance is running and skips.
3. Ensures the download directory exists.
4. Fetches the HTML directory listing via `wget`, extracts `.JPG` filenames matching the DashCam naming pattern, and downloads any file not already present locally.
